# Content Inventory & Ownership

Use this inventory before launch.

| Content Area | Owner | Approver | TR | EN | Media | Status |
|---|---|---|---|---|---|---|
| Homepage hero | TBD | TBD | TBD | TBD | TBD | Missing |
| Company stats | TBD | TBD | Known | Known | N/A | Partial |
| Corporate/About | TBD | TBD | TBD | TBD | TBD | Missing |
| Brands | TBD | TBD | TBD | TBD | Logos TBD | Missing |
| Product Groups | TBD | TBD | TBD | TBD | TBD | Missing |
| İstanbul location | TBD | TBD | TBD | TBD | TBD | Missing |
| Ankara location | TBD | TBD | TBD | TBD | TBD | Missing |
| Diyarbakır location | TBD | TBD | TBD | TBD | TBD | Missing |
| Careers intro | HR | TBD | TBD | TBD | TBD | Missing |
| Contact info | TBD | TBD | TBD | TBD | N/A | Missing |
| Privacy page | Legal/TBD | TBD | Temporary V1 | Temporary V1 | N/A | Legal review required |
| Cookie policy | Legal/TBD | TBD | Temporary V1 | Temporary V1 | N/A | Legal review required |
| KVKK/Data Protection | Legal/TBD | TBD | Temporary V1 | Temporary V1 | N/A | Legal review required |
| Career short notice | HR + Legal/TBD | TBD | Temporary V1 | Temporary V1 | N/A | Legal review required |
| Contact short notice | Contact + Legal/TBD | TBD | Temporary V1 | Temporary V1 | N/A | Legal review required |

## Required Metadata

For each public image:

- rights/permission source,
- owner/source,
- alt text TR/EN,
- focal point,
- intended placement.

## Rule

Do not treat missing content as permission to invent business facts.

Use placeholders only in development.
