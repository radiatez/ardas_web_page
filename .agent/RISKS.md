...
| R-030 | Generated temporary imagery is mistaken for approved/licensed documentary company photography | Medium | Low | Unbranded project-generated asset manifest with `temporaryMedia`/`requiresReplacement`, neutral localized alt text, no company/facility claim, non-immutable replacement path and explicit readiness register | Mitigated in presentation/code; final approved photography remains external |
...
| R-033 | Temporary legal copy is mistaken for lawyer-approved final copy | High | Medium | Explicit temporary/version/review metadata, admin-only warning, noindex seed, immutable revisions, approved-version/reference gate and production forms fail closed | Partially mitigated; lawyer approval and verified controller details pending |
